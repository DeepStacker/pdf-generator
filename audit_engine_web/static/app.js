// --- PYWEBVIEW ZERO-SOCKET IPC FETCH INTERCEPTOR ---
            ...
            if (files && files.length > 0) {
                if (isWebMode()) {
                    ...
                    if (paths.length > 0) {
                        ...
                        showToast("File upload failed. Please try using the browse button.", "warning");
                    }
                    return;
                }
                const paths = [];
        ...
        async function handleNativeFileSelect(e) {
            const files = e.target.files;
            if (files && files.length > 0) {
                if (isWebMode()) {
                    const paths = await uploadMultipleFilesToServer(files);
                    if (paths.length > 0) addSelectedFiles(paths);
                    e.target.value = '';
                    return;
                }
                const paths = [];
        ...
        async function browseFolder(fieldId) {
            if (isWebMode()) {
                showToast('Output directory is managed by the server in web mode.', 'info');
                return;
            }
            try {
        ...
        function openSummaryModal(summaryData) {
            ...
            if (summaryData.items && summaryData.items.length > 0) {
                ...
                const stagingItem = summaryData.items.find(i => i.label === 'Staging Directory' || i.label === 'Output Directory');
                if (isWebMode() && stagingItem) {
                    html += `<div class="py-3 mt-2"><a href="${getDownloadUrl(stagingItem.value)}" class="block w-full text-center px-4 py-2 bg-blue-600 hover:bg-blue-500 text-white rounded-lg font-bold text-sm transition" download>Download Output Files</a></div>`;
                }
            } else {
                html = `<div class="text-slate-400 italic">${summaryData.message || 'Workflow finished successfully.'}</div>`;
                if (isWebMode() && summaryData.output_path) {
                    html += `<div class="py-3 mt-2"><a href="${getDownloadUrl(summaryData.output_path)}" class="block w-full text-center px-4 py-2 bg-blue-600 hover:bg-blue-500 text-white rounded-lg font-bold text-sm transition" download>Download Output Files</a></div>`;
                }
            }
                ...
                data.forEach(row => {
                    ...
                    html += `
                        <tr class="hover:bg-slate-900/30 transition text-xs border-b border-brand-borderLine">
                            ...
                            <td class="py-3.5 px-6 text-center">${dlBtn}</td>
                        </tr>`;
                });
                body.innerHTML = html;
                updateThemeBranding();
        ...
        async function uploadFileToServer(file) {
            ...
            try {
                const resp = await originalFetch.call(window, '/api/upload', { method: 'POST', body: formData });
                const data = await resp.json();
        ...
        async function uploadMultipleFilesToServer(files) {
            const formData = new FormData();
            for (const f of files) formData.append('file', f);
            try {
                const resp = await originalFetch.call(window, '/api/upload/multiple', { method: 'POST', body: formData });
                const data = await resp.json();
