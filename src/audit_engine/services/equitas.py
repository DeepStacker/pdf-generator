"""Core audit logic for Equitas Small Finance Bank.
...
The EquitasService class provides a clean API. Module-level legacy functions
...
# =========================================================
# EquitasService class
# =========================================================

class EquitasService:
    """Service for Equitas Small Finance Bank audit processing.

    Provides both Stage 1 (master Excel → branch PDF/Excel) and Stage 2
    (consolidation → account-level report) workflows.

    All public helpers are available as @staticmethod so they can be called
    on the class directly or on an instance.
    """

    name = "Equitas Small Finance Bank"
    ROWS_PER_PAGE = ROWS_PER_PAGE
    NORMAL_COLUMNS = dict(NORMAL_COLUMNS)
    JSR_COLUMNS = dict(JSR_COLUMNS)
    ...
    def validate_stage1_file(self, file_path: str) -> tuple:
        ...
        try:
            ...
            for sheet in sheet_names:
                base = self.base_sheet_name(sheet)
    ...
    def validate_stage2_file(self, file_path: str) -> tuple:
        if not file_path or not str(file_path).strip():
            return False, "No file path provided."
    ...
    # --- Stage 1 — Master dataframe --------------------------------------

    def _read_sheet_dynamic(self, file_path, sheet_name, required_columns):
    ...
    def _validate_required_columns(self, df, required_columns, sheet_name):
        missing = [col for col in required_columns if col not in df.columns]
        if missing:
            raise ValueError(f"Missing columns in {sheet_name}: {missing}")

    def build_master_dataframe(self, file_path: str,
                               log_callback: Callable = print) -> pd.DataFrame:
        excel = pd.ExcelFile(file_path)
        ...
        try:
            ...
            for base, sheets in sheet_map.items():
                ...
                try:
                    ...
                    required_normal = [
                        self.NORMAL_COLUMNS["loan_no"],
                        self.NORMAL_COLUMNS["sole_id"],
                        self.NORMAL_COLUMNS["branch"],
                    ]
                    required_jsr = [v for v in self.JSR_COLUMNS.values()]

                    normal_df = self._read_sheet_dynamic(
                        file_path, normal_sheet, required_normal
                    )
                    jsr_df = self._read_sheet_dynamic(
                        file_path, jsr_sheet, required_jsr
                    )

                    self._validate_required_columns(normal_df, required_normal, normal_sheet)
                    self._validate_required_columns(jsr_df, required_jsr, jsr_sheet)

                    merged = pd.merge(
                        normal_df[required_normal].copy(),
                        jsr_df[required_jsr].copy(),
                        left_on=self.NORMAL_COLUMNS["loan_no"],
                        right_on=self.JSR_COLUMNS["loan_no"],
                        how="left",
                    )

                    merged["_GROUP_KEY"] = (
                        merged[self.NORMAL_COLUMNS["loan_no"]].astype(str).str.strip()
                    )
    ...
    def _create_pdf_table_data(self, rows, branch_name, verification_id,
                               branch_sole_id, page_no, total_pages, total_accounts):
        data = []

        data.append([
            Paragraph("<b>BRANCH NAME</b>", _header_style), "",
            Paragraph(self.safe_value(branch_name), _header_style),
        ...
        data.append([
            ...
            "", "", "", "", "",
            Paragraph("<b>Verification Details</b>", _header_style),
            "", "", "", "", "", "", "",
        ])
        ...
        for _, row in rows.iterrows():
            customer_name = self.clean_text(row.get(self.JSR_COLUMNS["customer"], ""))
            ornament = self.clean_text(row.get(self.JSR_COLUMNS["ornament"], ""))
            ...
            data.append([
                ...
                Paragraph(self.safe_float(row.get(self.JSR_COLUMNS["quantity"], "")), _body_style),
                Paragraph(self.safe_float(row.get(self.JSR_COLUMNS["gross_weight"], "")), _body_style),
                "", "", "", "", "", "", "", "",
            ])

        return data

    def generate_branch_pdf(self, branch_name: str, branch_df: pd.DataFrame,
                            output_dir: str) -> str:
        ...
        total_pages = (total_rows // self.ROWS_PER_PAGE) + (
            1 if total_rows % self.ROWS_PER_PAGE else 0
        )

        verification_id = self.generate_verification_id()
        branch_sole_id = self.safe_value(
            branch_df.iloc[0].get(self.NORMAL_COLUMNS["sole_id"], "")
        )

        page_no = 1
        for start in range(0, total_rows, self.ROWS_PER_PAGE):
            end = start + self.ROWS_PER_PAGE
            page_rows = branch_df.iloc[start:end]

            table_data = self._create_pdf_table_data(
    ...
    def generate_branch_excel(self, branch_name: str, branch_df: pd.DataFrame,
                              output_dir: str) -> str:
        os.makedirs(output_dir, exist_ok=True)
        ...
        for _, row in branch_df.iterrows():
            values = [
                row["SR_NO"],
                self.safe_value(row.get(self.NORMAL_COLUMNS["loan_no"], "")),
                self.clean_text(row.get(self.JSR_COLUMNS["customer"], "")),
                self.safe_float(row.get(self.JSR_COLUMNS["sanctioned_amount"], "")),
                self.format_date(row.get(self.JSR_COLUMNS["loan_date"], "")),
                self.safe_value(row.get(self.JSR_COLUMNS["old_packet_no"], "")),
                "",
                self.safe_float(row.get(self.JSR_COLUMNS["quantity"], "")),
                "",
                f'=IF(I{row_num}="","",ROUND(I{row_num}-H{row_num},3))',
                self.safe_float(row.get(self.JSR_COLUMNS["gross_weight"], "")),
                "",
                f'=IF(L{row_num}="","",ROUND(L{row_num}-K{row_num},3))',
                self.safe_float(row.get(self.JSR_COLUMNS["net_weight"], "")),
                "",
                f'=IF(O{row_num}="","",ROUND(O{row_num}-N{row_num},3))',
                self.safe_value(row.get(self.JSR_COLUMNS["carat_master"], "")),
                "",
                ...
                self.clean_text(row.get(self.JSR_COLUMNS["ornament"], "")),
            ]
        ...
        for cond_col in ["J", "M", "P"]:
            ws.conditional_formatting.add(
                f"{cond_col}5:{cond_col}{row_num}",
                FormulaRule(formula=[f'{cond_col}5<>0'], fill=red_fill),
            )
        ws.conditional_formatting.add(
            f"S5:S{row_num}",
    ...
    def run_stage1(
        ...
        master_df = self.build_master_dataframe(file_path, log_callback)

        branch_col = self.NORMAL_COLUMNS["branch"]
        ...
        for branch_name, branch_df in grouped:
            ...
            try:
                ...
                if output_format in ("PDF ONLY", "BOTH"):
                    pdf_path = self.generate_branch_pdf(branch_name, branch_df, pdf_output_dir)
                    log_callback(f"   PDF: {os.path.basename(pdf_path)}")
                    pdf_count += 1

                if output_format in ("EXCEL ONLY", "BOTH"):
                    excel_path = self.generate_branch_excel(branch_name, branch_df, excel_output_dir)
    ...
    def _avg_int(series):
        ...
        for x in series:
            try:
                val = EquitasService.safe_float_numeric(x)
                if val == 0:
                    continue
                vals.append(val)
            except (ValueError, TypeError):
    ...
    def _merge_issue_only(series):
        vals = []
        for x in series:
            s = EquitasService._safe_str(x)
            if not s:
                continue
            if s.lower() == "ok - no discrepancy":
                continue
            if s not in vals:
                vals.append(s)
    ...
    def load_stage1_excel(self, file_path: str) -> pd.DataFrame:
        df_top = pd.read_excel(file_path, header=None, nrows=20)
        header_idx = 3
        for idx, row in df_top.iterrows():
            row_str = [str(x).strip().upper() for x in row.values if pd.notna(x)]
    ...
    def consolidate_accounts(self, df: pd.DataFrame,
                             log_callback: Callable = print) -> pd.DataFrame:
        ...
        for acc_no, grp in grouped:
            try:
                sanction_amt = self.safe_float_numeric(
                    self._first_non_empty(grp["SANCTION AMT"])
                )

                bank_qty = round(grp["BANK QTY"].apply(self.safe_float_numeric).sum(), 3)
                actual_qty = round(grp["ACTUAL QTY"].apply(self.safe_float_numeric).sum(), 3)

                bank_gross = round(grp["BANK GROSS"].apply(self.safe_float_numeric).sum(), 3)
                actual_gross = round(grp["ACTUAL GROSS"].apply(self.safe_float_numeric).sum(), 3)

                bank_net = round(grp["BANK NET"].apply(self.safe_float_numeric).sum(), 3)
                actual_net = round(grp["ACTUAL NET"].apply(self.safe_float_numeric).sum(), 3)

                remarks = self._merge_issue_only(grp["REMARKS"])
                additional_details = self._merge_non_empty(grp["ADDITIONAL DETAILS"])
                avg_carat = self._avg_int(grp["ACTUAL CARAT"])

                rows.append({
                    "S.NO": sr_no,
                    "ACCOUNT NUMBER": self._safe_str(acc_no),
                    "APPLICANT NAME": self._first_non_empty(grp["APPLICANT NAME"]),
                    "SANCTIONED_AMOUNT": self.format_3(sanction_amt),
                    "DATE_OF_LOAN": self._first_non_empty(grp["LOAN DATE"]),
                    "OLD PACKET NO.": self._first_non_empty(grp["OLD PKT NO"]),
                    "NEW PACKET NO.": self._first_non_empty(grp["NEW PKT NO"]),
                    "Bank Valuation No. Of Ornaments": int(round(bank_qty)),
                    "Actual No. of Ornaments": int(round(actual_qty)),
                    "Bank Valuation Gross Wt.": self.format_3(bank_gross),
                    "Actual Gross Wt.": self.format_3(actual_gross),
                    "Bank Valuation Net Wt.": self.format_3(bank_net),
                    "Actual Net Wt.": self.format_3(actual_net),
                    "REMARKS": remarks,
    ...
    def generate_stage2_excel(self, original_file: str,
        ...
        for idx, h in enumerate(headers, start=1):
            ...
            c.border = border
            c.alignment = center
            c.fill = yellow_fill if idx <= _HEADER_COLUMNS_YELLOW else green_fill
    ...
    def run_stage2(
        ...
        df = self.load_stage1_excel(file_path)
        ...
        consolidated_df = self.consolidate_accounts(df, log_callback)
        ...
        output_path = self.generate_stage2_excel(file_path, consolidated_df, output_dir)
...
def normalize_columns(df):
    return EquitasService.normalize_columns(df)

def clean_text(value):
    return EquitasService.clean_text(value)

def safe_value(value):
    return EquitasService.safe_value(value)

def safe_float(value):
    return EquitasService.safe_float(value)

def safe_float_numeric(value):
    return EquitasService.safe_float_numeric(value)

def format_date(value):
    return EquitasService.format_date(value)

def base_sheet_name(sheet_name):
    return EquitasService.base_sheet_name(sheet_name)

def generate_verification_id():
    return EquitasService.generate_verification_id()

def sanitize_filename(name):
    return EquitasService.sanitize_filename(name)

def format_3(value):
    return EquitasService.format_3(value)


# =========================================================
# Default instance & backward-compatible public API
# =========================================================

_default_service = EquitasService()


def validate_equitas_stage1_file(file_path):
    return _default_service.validate_stage1_file(file_path)


def validate_equitas_stage2_file(file_path):
    return _default_service.validate_stage2_file(file_path)


def build_master_dataframe(file_path, log_callback=print):
    return _default_service.build_master_dataframe(file_path, log_callback)


def generate_branch_pdf(branch_name, branch_df, output_dir):
    return _default_service.generate_branch_pdf(branch_name, branch_df, output_dir)


def generate_branch_excel(branch_name, branch_df, output_dir):
    return _default_service.generate_branch_excel(branch_name, branch_df, output_dir)


def run_equitas_stage1(file_path, output_dir, log_callback=print, cancel_event=None,
                       progress_callback=None, output_format="BOTH", output_mode="FOLDER"):
    return _default_service.run_stage1(
        file_path, output_dir, log_callback, cancel_event,
        progress_callback, output_format, output_mode,
    )


def load_stage1_excel(file_path):
    return _default_service.load_stage1_excel(file_path)


def consolidate_accounts(df, log_callback=print):
    return _default_service.consolidate_accounts(df, log_callback)


def generate_stage2_excel(original_file, consolidated_df, output_dir):
    return _default_service.generate_stage2_excel(original_file, consolidated_df, output_dir)


def run_equitas_stage2(file_path, output_dir, log_callback=print,
                       cancel_event=None, progress_callback=None):
    return _default_service.run_stage2(
        file_path, output_dir, log_callback, cancel_event, progress_callback,
    )
